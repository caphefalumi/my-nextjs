    ...
    async (file: File) => {
      ...
      try {
        ...
        if (useBackend && extension === "csv") {
          ...
          // Create ParsedData from response for preview only (store is already updated by uploadFile)
          const data: ParsedData = {
            ...
            rows: response.employees.map((emp) => ({
              ...
              name: emp.name,
              role: emp.role,
              department: emp.role, // Backend doesn't have department, use role
              email: `${emp.name.toLowerCase().replace(' ', '.')}@company.com`,
              impactScore: emp.impact_score, // Use snake_case from backend
          ...
          setStatus("success");
          // Note: Don't call onDataParsed here - uploadFile already updated the store
          // We just set the parsed data for preview purposes
        } else {
